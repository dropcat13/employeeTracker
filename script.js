...
function addQuestions() {
    inquirer
      ...
      .then(function(answer){
          ...
          if (answer.activity === "DEPARTMENTS") {
            addDepartments();
            }
            else if(answer.activity === "ROLES") {
            addRoles();
            } 
            else if(answer.activity === "EMPLOYEES") {
            addEmployees();
            } 
            else{
            connection.end();
...
function viewQuestions() {
    inquirer
      .prompt({
        type: "list",
          name: "viewType",
          message: "Do you want to view the records of [DEPARTMENTS], [ROLES] or [EMPLOYEES]?",
          choices: ["ADD", "VIEW", "UPDATE", "EXIT"]
      })
      .then(function(answer){
        // based on their answer, either call the add departments, add roles or add employees functions
...
function viewDepartments() {
    inquirer
      ...
      .then(connection.query("SELECT * FROM departments", function(err, res) {
         if (err) throw err;
...
function viewRoles() {
    inquirer
      .prompt({
        type: "input",
          name: "roleNameAnswer",
          message: "Please enter the name of the role you wish to view."     
      })
      .then(connection.query("SELECT * FROM roles", function(err, res) {
         if (err) throw err;
...
function viewEmployees() {
    inquirer
      .prompt({
        type: "input",
          name: "employeeNameAnswer",
          message: "Please enter the name of the employee you wish to view."     
      })
      .then(connection.query("SELECT * FROM employees", function(err, res) {
         if (err) throw err;
